...
! integration schemes
integer, parameter :: FORWARD_EULER = 1, MODIFIED_EULER = 2, RK4 = 3
...
integer :: Nsteps
integer :: extra_header_frequency, extra_terminal_frequency, integration_scheme

!constant parameters
real(dp), parameter :: v_rel_const = 0, rho_amb = 1d-10

!print variable
real(dp) :: print_vals(12)
...
subroutine advance_energy_prescription(s, t, dt)
   ...
   real(dp) :: xstar(5), dxstardt(5)
   real(dp) :: k1(5), k2(5), k3(5), k4(5)

   ! set up solution vector
   x = (/ s%xtra(ia), s%xtra(iomega), s%xtra(iM_ns), s%xtra(iM_acc) , 0d0 /)

   ! calculate the right-hand-side of the differential equations
   call energy_prescrip_rhs(s, t, x, dxdt)

   ! integrate
   select case (integration_scheme)
      case(FORWARD_EULER)           !forward Euler
         x = x + dt*dxdt
      case(MODIFIED_EULER)          !modified Euler
         xstar = x + dt*dxdt
         call energy_prescrip_rhs(s, t+dt, xstar, dxstardt)
         x = x + dt/2*(dxdt + dxstardt)
      case(RK4)                     !RK4
         k1 = dxdt
         call energy_prescrip_rhs(s, t+dt/2, x+k1*dt/2, k2)
         call energy_prescrip_rhs(s, t+dt/2, x+k2*dt/2, k3)
         call energy_prescrip_rhs(s, t+dt  , x+k3*dt  , k4)
...
subroutine advance_force_prescription(s, t, dt)
   ...
   real(dp) :: k1(12), k2(12), k3(12), k4(12)
   ...
   select case (integration_scheme)
      ...
      case(RK4)
         k1 = dxdt
         call force_prescrip_rhs(s, t+dt/2, x+k1*dt/2, k2)
         call force_prescrip_rhs(s, t+dt/2, x+k2*dt/2, k3)
         call force_prescrip_rhs(s, t+dt  , x+k3*dt  , k4)
         x = x + dt/6 * (k1 + 2*k2 + 2*k3 + k4)
   end select
...
subroutine energy_prescrip_rhs(s, t, x, dxdt)
   ...
   ! Ignore fd and edot from this model if drag prescription is not MR15
   call mr15(s, a_orb, mns, rho, v_rel, mdot, fd, edot)

   ! if (drag_prescription /= MR15_DRAG) then we need
   ! to get fd and edot from the chosen drag prescription
   ! the accretion rate is still calculated using MR15
...
subroutine get_e_beta_given_omega(M, omega, e, beta)
   ...
   real(dp), parameter :: tol = 1.e-6, kappan = 1 !Lai+ 93 table 1
   ...
   do while ((abs(onxt) > tol) .and. (n_iter < max_iter))
      enxt = (emin + emax) / 2
      onxt = (sqrt(1-enxt**2) * (3-2*enxt**2) * asin(enxt) / enxt**3 - 3 * (1-enxt**2)/enxt**2) - ofctn_val
                                                      ! Holgado et al. eqn 35
      if (onxt > 0.) then
         emax = enxt
      else
...
subroutine get_spinup_rate(s, M, Macc, Mdot, omega, omegadot, e, beta, Q)
   ...
   real(dp)            :: Rbar, Req, Rz, Re, Imom, Qtb, Qmax, Nacc, Ngw, tau
   ...
   Rz = R0 * (1 - e/2)
   Re = R0 * (1 + e/2)
   Qtb  = sqrt(5 * clight**5 * Mdot * sqrt(standard_cgrav*M*Req) / (32*standard_cgrav*omega**5))               ! Holgado et al. eqn 19
   Qmax = 4d39*(1 - Macc/M_crust)                  ! Holgado et al. eqn 40
   Imom     = (M/5.) * (Rz**2 + Re**2)                          ! Holgado et al. after eqn 16  !(M/5.) * (Rz**2 + Req**2)
...
subroutine extras_startup(id, restart, ierr)
   ...
   if (ierr /= 0) return
      ...
      integration_scheme = int(s%x_ctrl(20))  !integration scheme to use for the orbit evolution
...
!TODO
   ...
   print_vals(1) = vals(1)
   print_vals(3) = vals(10)
   print_vals(4) = vals(9)
   print_vals(5) = vals(12)
   print_vals(9) = vals(2)
   print_vals(10) = vals(3)
   print_vals(11) = vals(5)
   print_vals(12) = vals(4)
   ...
   select case (orbit_prescription)
      case (FORCE_PRESCRIPTION)
         ...
         print_vals(2) = sv_rel
         print_vals(6) = vals(36)
         print_vals(7) = vals(37)
         print_vals(8) = vals(33)
      ...
      case (ENERGY_PRESCRIPTION)
         ...
         print_vals(2) = sv_rel
         print_vals(6) = vals(26)
         print_vals(7) = vals(27)
         print_vals(8) = vals(23)
...
integer function extras_finish_step(id)
   ...
   if (mod(s% model_number, extra_terminal_frequency) == 0 .or. (s% model_number == 1)) then
      write(*,'(A1,ES10.3,11ES11.3,A3)') &
         '|', &
         s%xtra(ia), sv_rel, s%xtra(iomega), s%xtra(iQ), &
         s%xtra(istrain), sfd, sedot, smdot, &
         se_inj, s%xtra(iM_ns), s%xtra(iM_acc), smenc, &
         '|'

      ! write(*,'(A1,ES10.3,11ES11.3,A3)') &
      !    '|', &
      !    print_vals(1)/Rsun, print_vals(2), print_vals(3)/2, print_vals(4), &
      !    print_vals(5), print_vals(6), print_vals(7), print_vals(8), &
      !    print_vals(9), print_vals(10)/Msun, print_vals(11)/Msun, print_vals(12)/Msun, &
      !    '|'
