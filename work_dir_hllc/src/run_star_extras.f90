...
!constant parameters
real(dp), parameter ::  ia_init_rsun = 400, Qmax_const = 4*1d39, &
...
subroutine add_energy_to_mesa(s, r, mns, v_rel, e_inj)
   ...
   !s%extra_heat(j)%val = efactor * temp_heat(j) * e_inj / (kernel_sum * s%dt_next) 
   enddo
...
!TODO
   ...
   if (prescription == FORCE_PRESCRIPTION) then
      ...
      call get_relative_velocity_energy_rhs(s, vals(1), vals(3), vals(18))
...
subroutine data_for_extra_profile_header_items(id, n, names, vals, ierr)
   ...
   if (prescription == FORCE_PRESCRIPTION) then
      ...
      call get_relative_velocity_energy_rhs(s, vals(1), vals(3), vals(20))
   ...
   if (vals(1) > s%R(1)) then
      vals(6) = vals(6) + rho_amb * (4/3) * pi * (vals(1)**3 - s%R(1)**3)
      vals(8) = rho_amb
      vals(15) = s%scale_height(1)
      vals(16) = sqrt(5 * s%lnPgas(1) / (3 * vals(6))) !assuming ideal gas, isentropic, gamma = 5/3
      vals(17) = s%lnPgas(1)
      vals(18) = s%lnT(1)
   end if
